...
class LUDecomposition:
    ...
    def multiply_matrices(A, B):
        ...
        m = len(B[0])
        p = len(B)
        result = LUDecomposition.zero_matrix(n, m)
    ...
    def lu_decomposition_with_pivoting(A):
        n = len(A)
        U = LUDecomposition.copy_matrix(A)
        L = LUDecomposition.identity_matrix(n)
        P = LUDecomposition.identity_matrix(n)
        swap_count = 0
        ...
        for k in range(n - 1):
            ...
            if max_value < LUDecomposition.EPS:
                raise ValueError("Матрица вырождена или близка к вырожденной")

            if pivot_row != k:
                LUDecomposition.swap_rows(U, k, pivot_row)
                LUDecomposition.swap_rows(P, k, pivot_row)
        ...
        if abs(U[n - 1][n - 1]) < LUDecomposition.EPS:
            raise ValueError("Матрица вырождена или близка к вырожденной")
    ...
    def backward_substitution(U, y):
        ...
        for i in range(n - 1, -1, -1):
            if abs(U[i][i]) < LUDecomposition.EPS:
    ...
    def solve_system(A, b):
        L, U, P, _ = LUDecomposition.lu_decomposition_with_pivoting(A)
        b_permuted = LUDecomposition.apply_permutation(P, b)
        y = LUDecomposition.forward_substitution(L, b_permuted)
        x = LUDecomposition.backward_substitution(U, y)
        return x

    @staticmethod
    def calculate_determinant(A):
        _, U, _, swap_count = LUDecomposition.lu_decomposition_with_pivoting(A)
    ...
    def inverse_matrix(A):
        n = len(A)
        inv = LUDecomposition.zero_matrix(n, n)

        for i in range(n):
            e = [0.0] * n
            e[i] = 1.0
            col = LUDecomposition.solve_system(A, e)
    ...
    def calculate_residual(A, x, b):
        Ax = LUDecomposition.multiply_matrix_vector(A, x)
        return LUDecomposition.subtract_vectors(Ax, b)
...
class ResultsWidget(QWidget):
    ...
    def display_results(self, x, residual, det, inv_A):
        ...
        self.clear_bars()
        max_val = LUDecomposition.max_abs_in_vector(x)
        if max_val == 0:
            max_val = 1.0
        ...
        max_residual = LUDecomposition.max_abs_in_vector(residual)

        if max_residual < 1e-10:
            self.residual_status.setText("Отлично! Невязка в пределах машинной точности")
            self.residual_status.setStyleSheet(
                "background-color: #c8e6c9; color: #2e7d32; padding: 10px; border-radius: 5px;"
            )
        elif max_residual < 1e-6:
            self.residual_status.setText("Хорошо, но есть небольшая погрешность")
            self.residual_status.setStyleSheet(
                "background-color: #fff3e0; color: #e65100; padding: 10px; border-radius: 5px;"
            )
        else:
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        info_label = QLabel(
            ...
            "Размерность: 4 x 4\n"
            "Реализация метода вручную без использования numpy"
        )
