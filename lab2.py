...
class TridiagonalSolver:
    ...
    def solve(a, b, c, d):
        n = len(b)
        alpha = TridiagonalSolver.zero_vector(n)
        beta = TridiagonalSolver.zero_vector(n)

        if abs(b[0]) < TridiagonalSolver.EPS:
        ...
        for i in range(1, n):
            ...
            if abs(denom) < TridiagonalSolver.EPS:
                raise ValueError(f"Деление на ноль на шаге i={i + 1}")
        ...
        x = TridiagonalSolver.zero_vector(n)
        x[n - 1] = beta[n - 1]
    ...
    def inverse(a, b, c):
        n = len(b)
        inv = TridiagonalSolver.zero_matrix(n, n)

        for j in range(n):
            e = TridiagonalSolver.zero_vector(n)
            e[j] = 1.0

            col = TridiagonalSolver.solve(a, b, c, e)
    ...
    def build_full_matrix(a, b, c):
        n = len(b)
        A = TridiagonalSolver.zero_matrix(n, n)
    ...
    def multiply_matrix_vector(A, x):
        n = len(A)
        result = TridiagonalSolver.zero_vector(n)
    ...
    def residual(a, b, c, x, d):
        A = TridiagonalSolver.build_full_matrix(a, b, c)
        Ax = TridiagonalSolver.multiply_matrix_vector(A, x)
        return TridiagonalSolver.subtract_vectors(Ax, d)
...
class TridiagonalInputDialog(QDialog):
    ...
    def get_system(self):
        try:
            n = self.n
            a = TridiagonalSolver.zero_vector(n)
            b = TridiagonalSolver.zero_vector(n)
            c = TridiagonalSolver.zero_vector(n)
            d = TridiagonalSolver.zero_vector(n)
...
class ResultsWidget(QWidget):
    ...
    def display_results(self, a, b, c, d, x, residual, det, inv_A):
        ...
        max_val = TridiagonalSolver.max_abs_in_vector(x)
        if max_val == 0:
            max_val = 1.0
        ...
        max_residual = TridiagonalSolver.max_abs_in_vector(residual)
        ...
        self.det_formula.setText(
            "Определитель вычислен по рекуррентной формуле:\n"
            "det[1] = b[1]\n"
            "det[i] = b[i]*det[i-1] - a[i]*c[i-1]*det[i-2]"
        )

        n_inv = len(inv_A)
        self.inv_table.setRowCount(n_inv)
        self.inv_table.setColumnCount(n_inv)
        self.inv_table.setHorizontalHeaderLabels([f"x{j+1}" for j in range(n_inv)])
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        info_label = QLabel(
            ...
            "Реализация выполнена вручную без использования numpy"
        )
    ...
    def solve_and_display(self, a, b, c, d):
        try:
            ...
            residual = TridiagonalSolver.residual(a, b, c, x, d)
