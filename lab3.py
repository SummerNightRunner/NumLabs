...
class IterativeSolvers:
    ...
    def simple_iteration(A, b, tolerance, max_iterations=10000):
        ...
        for iteration in range(max_iterations):
            ...
            if error < tolerance:
                return x_new, iterations + 1, errors, True
            
            x = x_new
            iterations += 1
        
        return x, max_iterations, errors, False
    
    @staticmethod
    def seidel_method(A, b, tolerance, max_iterations=10000):
        """
        Метод Зейделя
        Использует уже обновлённые значения на текущей итерации
        """
        n = len(A)
        x = np.zeros(n)
        errors = []
...
class ToleranceDialog(QDialog):
    """Диалог для выбора метода и точности"""
    ...
    def init_ui(self):
        ...
        # Создаем контейнеры для радио-кнопок с фоном
        ...
        # Делаем кнопки шире и с отступами
...
class ResultsWidget(QWidget):
    ...
    def setup_iterations_tab(self):
        ...
        self.iter_plot_label = QLabel("График сходимости (логарифмическая шкала)")
    ...
    def display_results(self, method_name, x, iterations, errors, residual, converged):
        ...
        # Таблица итераций
        self.iter_table.setRowCount(min(len(errors), 50))
        for i in range(min(len(errors), 50)):
...
class MainWindow(QMainWindow):
    ...
    def solve_and_display(self, method, tolerance):
        try:
            ...
            if method == "simple":
                # Метод простых итераций
                x, iterations, errors, converged = IterativeSolvers.simple_iteration(
                    self.A, self.b, tolerance
                )
                method_name = "Метод простых итераций (Якоби)"
                
                # Также решаем методом Зейделя для сравнения
                x_seidel, iter_seidel, errors_seidel, converged_seidel = IterativeSolvers.seidel_method(
                    self.A, self.b, tolerance
                )
                ...
                simple_data = {
                    ...
                    'converged': converged,
                    'residual': IterativeSolvers.residual(self.A, self.b, x)
                }
                
            else:
                # Метод Зейделя
                x, iterations, errors, converged = IterativeSolvers.seidel_method(
                    self.A, self.b, tolerance
                )
                method_name = "Метод Зейделя"
                
                # Также решаем методом простых итераций для сравнения
                x_simple, iter_simple, errors_simple, converged_simple = IterativeSolvers.simple_iteration(
                    self.A, self.b, tolerance
                )
            ...
            # Вычисляем невязку для выбранного метода
            residual = IterativeSolvers.residual(self.A, self.b, x if method == "simple" else x)
            
            # Отображаем результаты
            self.results_widget.display_results(method_name, x if method == "simple" else x, 
                                              iterations, errors, residual, converged)
